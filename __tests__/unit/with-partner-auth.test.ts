...
function req(headers: Record<string, string> = {}): NextRequest {
  return new NextRequest("https://x/api/v1/partner/depix/balance", { headers });
}
...
describe("withPartnerAuth", () => {
  it("401 sem Authorization", async () => {
    const r = await withPartnerAuth(req(), { scope: PARTNER_SCOPES.DEPIX_READ });
  ...
  it("401 key invalida", async () => {
    ...
    const r = await withPartnerAuth(req({ authorization: "Bearer at_xxx_yyy" }), { scope: PARTNER_SCOPES.DEPIX_READ });
    expect((r as Response).status).toBe(401);
  });

  it("403 sem o escopo exigido", async () => {
    validatePartnerApiKey.mockResolvedValue({
      tenantId: "t1", keyId: "k1", keyPrefix: "abcd1234", scopes: ["depix:read"],
    });
  ...
  it("429 acima da quota", async () => {
    ...
    const r = await withPartnerAuth(req({ authorization: "Bearer at_x_y" }), { scope: PARTNER_SCOPES.DEPIX_READ });
  ...
  it("ok: retorna o contexto do parceiro (tenant+scopes)", async () => {
    validatePartnerApiKey.mockResolvedValue({
      tenantId: "t1", keyId: "k1", keyPrefix: "abcd1234", scopes: ["depix:read", "depix:deposit"],
    });
    const r = await withPartnerAuth(req({ authorization: "Bearer at_x_y" }), { scope: PARTNER_SCOPES.DEPIX_READ });
    expect(r).not.toBeInstanceOf(Response);
    expect(r).toMatchObject({ tenantId: "t1", keyPrefix: "abcd1234" });
    // rate-limit por key+escopo.
    expect(rateLimit.mock.calls[0]![0]).toMatchObject({ key: "partner:abcd1234:depix:read" });
  });
});
