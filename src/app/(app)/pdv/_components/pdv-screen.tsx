...
import { Search, X, ShoppingCart, User, Tag, RotateCcw, CreditCard } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";
import { Card, CardContent, CardHeader, CardTitle } from "@/components/ui/card";
import { Dialog, DialogContent, DialogHeader, DialogTitle, DialogFooter, DialogDescription } from "@/components/ui/dialog";
import { Label } from "@/components/ui/label";
// No radio-group or toggle-group available - using buttons
import { EntitySelector } from "@/components/domain/entity-selector";
import { cn } from "@/lib/utils";
import { toast } from "@/lib/toast";
// PAYMENT_METHOD_LABELS used in payment-dialog
...
export function PdvScreen() {
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const draft = draftQuery.data as Record<string, any> | undefined;
  const items = ((draft?.items ?? []) as Array<{
    id: string;
    productId: string;
    description: string;
    quantity: number;
  ...
  const updateItemMutation = useMutation(trpc.sale.updateItemQuantity.mutationOptions());
  const removeItemMutation = useMutation(trpc.sale.removeItem.mutationOptions());
  const applyDiscountMutation = useMutation(trpc.sale.applyDiscount.mutationOptions());
  const setCustomerMutation = useMutation(trpc.sale.setCustomer.mutationOptions());

  const invalidateDraft = useCallback(() => {
    if (draftId) {
      queryClient.invalidateQueries({ queryKey: trpc.sale.getDraft.queryKey({ id: draftId }) });
    }
    ...
    applyDiscountMutation.mutate(
      {
        saleId: draftId,
        discountType,
        discountValue: discountType === "fixed" ? Math.round(value * 100) : value,
  ...
  const subtotal = draft?.subtotal as number ?? 0;
  const discountAmount = draft?.discountAmount as number ?? 0;
  const totalAmount = draft?.totalAmount as number ?? 0;
    ...
    <div className="grid grid-cols-1 lg:grid-cols-[1fr_350px] gap-4 h-[calc(100vh-80px)]">
      ...
      <div className="flex flex-col min-h-0">
        ...
        <div className="flex items-center justify-between mb-3">
          <h1 className="text-xl font-bold text-foreground">PDV - Ponto de Venda</h1>
                ...
                {items.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center py-12 text-muted-foreground">
                      ...
                      <p className="text-xs mt-1">Busque um produto acima ou pressione F2</p>
                    </td>
                  </tr>
                ) : (
                  items.map((item) => (
      ...
      <div className="flex flex-col gap-3">
          ...
          <CardContent className="px-4 pb-3">
            {customerId && customerName ? (
              <div className="flex items-center justify-between bg-muted/50 rounded-md p-2.5">
                ...
                <Button variant="ghost" size="sm" onClick={handleRemoveCustomer} className="text-xs shrink-0">
                  Trocar
                </Button>
              </div>
            ) : (
        ...
        <div className="flex flex-col gap-2 mt-auto">
          ...
          <div className="flex items-center justify-center gap-4 text-xs text-muted-foreground">
            <span><kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px] font-mono">F2</kbd> buscar</span>
            <span><kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px] font-mono">F8</kbd> finalizar</span>
            <span><kbd className="px-1.5 py-0.5 bg-muted rounded text-[10px] font-mono">F9</kbd> nova venda</span>
          </div>
          ...
          <Button variant="ghost" className="w-full" onClick={() => router.push("/pdv/history")}>
            Historico de Vendas
          </Button>
        </div>
      </div>
      ...
      <Dialog open={showCustomerDialog} onOpenChange={setShowCustomerDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Busque o cliente pelo nome ou CPF</DialogDescription>
          </DialogHeader>
          <EntitySelector
            value={customerId}
      ...
      <Dialog open={showDiscountDialog} onOpenChange={setShowDiscountDialog}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>Escolha o tipo e informe o valor</DialogDescription>
          </DialogHeader>
