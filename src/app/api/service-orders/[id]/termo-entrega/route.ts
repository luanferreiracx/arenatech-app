 ...
 *
 * Termo de Entrega — FAITHFUL to Laravel. Green (#28a745) theme.
 * Confirms client received equipment after service completion.
 * "Conferi o funcionamento"
 */
...
<title>Termo de Entrega - ${esc(order.number)}</title>
<style>
  body { font-family: Arial, sans-serif; font-size: 11pt; margin: 20px; }
  .cabecalho { border-bottom: 3px solid #28a745; padding-bottom: 15px; margin-bottom: 20px; }
  .titulo-doc { text-align: center; font-size: 18pt; font-weight: bold; margin: 20px 0; color: #28a745; background: #e8f5e9; padding: 15px; border-radius: 8px; }
  ...
  .equipamento-titulo { font-size: 14pt; font-weight: bold; color: #28a745; margin-bottom: 10px; }
  ...
  <div class="cabecalho">
    <table style="width: 100%; border: none;"><tr>
      ...
      <td style="vertical-align: middle;">
        <div style="font-size: 11pt; font-weight: bold; color: #28a745;">${esc(nomeLoja)}${cnpjLoja ? ` - CNPJ: ${esc(cnpjLoja)}` : ""}</div>
  ...
  <div class="titulo-doc">TERMO DE ENTREGA DE EQUIPAMENTO</div>

  <div class="info-box">
    <div class="info-row"><span class="label">Ordem de Servico:</span> ${esc(order.number)}</div>
    <div class="info-row"><span class="label">Data de Entrega:</span> ${now.toLocaleDateString("pt-BR")} ${now.toLocaleTimeString("pt-BR", { hour: "2-digit", minute: "2-digit" })}</div>
  ...
  <div class="equipamento-box">
    <div class="equipamento-titulo">EQUIPAMENTO ENTREGUE</div>
  ...
  <div class="texto-termo">
    <p>Declaro ter recebido o equipamento acima descrito, apos a realizacao dos servicos de
    assistencia tecnica conforme Ordem de Servico <strong>${esc(order.number)}</strong>.</p>

    <p>Declaro que conferi o funcionamento do equipamento no ato da entrega,
    nao tendo nenhuma reclamacao a fazer neste momento.</p>

    <p style="margin-top: 15px; font-size: 10pt; color: #666;">
      <em>* Informacoes sobre valor, garantia e detalhes do servico constam no Recibo de Servico.</em>
    </p>
  </div>
  ...
  <div class="rodape">
    ...
    logger.error("Termo entrega error:", { err: String(err) });
