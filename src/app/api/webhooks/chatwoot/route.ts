    ...
    switch (event) {
      case "message_created": {
        ...
        const persisted = await withAdmin(async (tx) => {
          ...
          // Em OPEN o bot NUNCA responde com IA (território do atendente) — o
          // acompanhamento de conversa parada fica por conta do cron de espera
          // (mensagem "aguarde" + alerta no grupo), não da IA.
