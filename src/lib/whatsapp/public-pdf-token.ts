...
export function createPublicPdfToken(tenantId: string, orderId: string, ttlMs = 60 * 60 * 1000): string {
...
export function verifyPublicPdfToken(token: string): PublicPdfTokenPayload | null {
  ...
  const [tenantId, orderId, expiresAtStr] = payload.split(".");
  if (!tenantId || !orderId || !expiresAtStr) return null;
  const expiresAt = Number(expiresAtStr);
  if (!Number.isFinite(expiresAt) || expiresAt < Date.now()) return null;

  return { tenantId, orderId, expiresAt };
}
